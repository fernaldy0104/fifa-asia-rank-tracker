...
# Koneksi ke Database menggunakan variabel REDIS_URL dari Settings Vercel
redis_url = os.environ.get('REDIS_URL')
kv = Redis.from_url(redis_url)

def get_initial_data():
    # Fungsi untuk mengambil data awal dari file JSON jika Redis masih kosong
...
@app.route('/')
def index():
    # 1. Ambil data dari database Redis
    teams_data_raw = kv.get('afc_teams')
    
    # 2. Jika database kosong (pertama kali jalan), isi dengan data dari JSON
    if not teams_data_raw:
        teams_data = get_initial_data()
        kv.set('afc_teams', json.dumps(teams_data))
    else:
        # Jika data ada, Redis mengembalikan string, ubah kembali ke list
...
def update():
    ...
    try:
        # Cari posisi tim yang akan diupdate
        ...
        # Update poin (Konstanta 25)
        teams_data[idx_tim]['poin'] = round(p_tim + 25 * (hasil - we), 2)

        # 4. SIMPAN KEMBALI KE REDIS (Penting agar data permanen)
        kv.set('afc_teams', json.dumps(teams_data))
